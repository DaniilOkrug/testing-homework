# Мои инструкции для тестирования

Модульные тесты запускаются с помощью команды `npm run test:unit`. Для тестирования с багами: `BUG_ID=1 npm run test:unit`.
Интеграционные тесты запускаются с помощью `npm run test:e2e`. ID бага меняется в `test\hermione\constants.js`.

Установка и запуск с помощью инструкций ниже.
